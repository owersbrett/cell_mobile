...
// ============================================================================
// TISSUE TYPE v2 — Classify the procedurally-drawn histology sample.
//
// v2 is a LIGHT-TOUCH refinement of the original `tissue_type`. The soul is
// preserved verbatim: the same four-answer taxonomy, the same uniform H&E stain
// (read SHAPE not colour), and the same procedural `_SamplePainter` histology —
// the brand asset of this game — carried over unchanged.
//
// THE ONE FLAW FIXED — clock-stopping fact flare:
//   v1 gated EVERY answer behind a 2.4s fact card (`_kFactFlareDuration`) that
//   the next sample waited on — ~15 forced micro-pauses that broke the 7→3s
//   buzzer ramp into a stop-start rhythm. v2 makes confirmation INSTANT: an
//   answer registers immediately, the slide flashes correct/wrong for a brief
//   ~0.4s reveal, then the next sample loads. The subtype fact is moved into a
//   persistent bottom TICKER that fades on its own life timer — the next sample
//   never waits on it. The climb to the buzzer is now smooth and accelerating.
//
// THE UNIVERSAL FAIRNESS LAYER — no runaway:
//   the streak multiplier is CAPPED at ×4 (v1 was uncapped: 1 + streak~/3 could
//   reach ×11 on a hot streak and run away from the field). Capped speed × capped
//   streak keeps scores comparable in pass-and-play.
//
// PERF: one Ticker drives everything through a single `_frame` notifier — NO
// per-frame setState over the widget tree. The heavy slide lives in its own
// RepaintBoundary + CustomPainter (repaints only when the sample id changes).
// The cheap ambient/particle layer and the slide overlay (deadline bar + reveal
// flash) repaint off `_frame`. setState fires ONLY on discrete transitions
// (tap / timeout / load-next).
// ============================================================================
...
// -- Timing / scoring --------------------------------------------------------
const double _kRevealDuration = 0.42; // short instant-feeling confirmation flash
const double _kFactLife       = 3.2;  // independent, non-blocking fact-ticker fade
const int    _kMaxPoints   = 130;
const int    _kFloorPoints = 25;
const int    _kStreakStep  = 3;   // every N correct → +1× multiplier
const int    _kMaxMult     = 4;   // fairness cap — streaks can't run away
...
// ============================================================================
// Tissue model (preserved from v1 — the accurate, on-brand content)
// ============================================================================
...
class TissueTypeV2Game extends StatefulWidget {
  final MiniGameSession session;
  const TissueTypeV2Game({super.key, required this.session});

  @override
  State<TissueTypeV2Game> createState() => _TissueTypeV2GameState();
}

class _TissueTypeV2GameState extends State<TissueTypeV2Game>
    with SingleTickerProviderStateMixin {
  late final Ticker _ticker;
  final math.Random _rng = math.Random();

  // Drives all per-frame painters WITHOUT rebuilding the widget tree.
  ...
  double _subtlety = 0;    // 0 = obvious tells … 0.82 = faint
  ...
  double _revealTimer = 0;    // short instant-confirmation flash window
  int _samplesShown = 0;

  int _streak = 0;
  int _shownMult = 1;

  // -- Fact ticker state (persists across the next sample; fades on its own
  //    life timer — NON-blocking, the loop never waits on it) ----------------
  _Tissue? _factTissue;
  int _factSubtype = 0;
  String _factText = '';
  bool _factCorrect = false;
  bool _factTimedOut = false;
  int _factEarned = 0;
  int _factMult = 1;
  double _factLife = 0;
  ...
  void _loadNext() {
    ...
    _sampleTime = 0;
    _revealTimer = 0;
    // NOTE: fact ticker fields are intentionally NOT reset here — the previous
    // fact lingers and fades on its own while this new sample is already live.
  }
  ...
  void _onTick(Duration elapsed) {
    ...
    // Per-frame painters repaint off this; the widget tree does NOT rebuild.
    _frame.value++;
  }
  ...
  void _simulate(double dt) {
    if (_answer == _Answer.waiting) {
      ...
      // Brief instant-confirmation flash, then immediately advance. The fact
      // ticker keeps fading independently — the next sample never waits on it.
      _revealTimer -= dt;
      if (_revealTimer <= 0) {
        _samplesShown++;
        setState(_loadNext);
    ...
    // Fact ticker fades on its own clock (non-blocking).
    if (_factLife > 0) _factLife -= dt;
  ...
  int _streakMult() =>
      (1 + (_streak ~/ _kStreakStep)).clamp(1, _kMaxMult);
  ...
  void _onTap(_Tissue choice) {
    ...
    setState(() {
      ...
      if (choice == _sample.tissue) {
        ...
        _shownMult = 1;
        _earnedPts = 0;
        _burst(_field.center(Offset.zero), _kBadRed, count: 8);
      }
      _setFact();
      _revealTimer = _kRevealDuration;
    });
  }
  ...
  /// Snapshot the just-answered sample into the non-blocking fact ticker.
  void _setFact() {
    _factTissue = _sample.tissue;
    _factSubtype = _sample.subtype;
    _factText = _sample.fact;
    _factCorrect = _answer == _Answer.correct;
    _factTimedOut = _tapped == null && !_factCorrect;
    _factEarned = _earnedPts;
    _factMult = _shownMult;
    _factLife = _kFactLife;
  }
  ...
  Widget build(BuildContext context) {
    return LayoutBuilder(builder: (context, constraints) {
      ...
      return ClipRect(
        child: Stack(
          children: [
            // Ambient field + particles — repaints off _frame, never rebuilds.
            Positioned.fill(
              child: CustomPaint(
                painter: _AmbientPainter(this, _frame),
              ),
            ),
      ...
      child: LayoutBuilder(builder: (context, c) {
        final content = Column(
          children: [
            ...
            // Persistent fact ticker — fixed slot, no layout shift, never blocks.
            _buildTicker(),
          ],
        );

        final bool tooShort = c.maxHeight < 500;
        if (tooShort) {
          return SingleChildScrollView(
            child: ConstrainedBox(
              constraints: BoxConstraints(
                minHeight: 500,
                maxHeight: math.max(500, c.maxHeight),
              ),
              child: content,
            ),
          );
  ...
  Widget _buildHUD() {
    // Rebuilds when the session notifies (score / host countdown). The mult
    // badge reads the latest internal streak state at that time.
            ...
            children: [
              ...
              Text(
                ...
                style: TextStyle(
                  ...
                  color: secs < 5
                      ? _kBadRed
                      : _kTextPri.withValues(alpha: 0.82),
                  shadows: secs < 5
      ...
      child: ClipRRect(
        ...
        child: Stack(
          children: [
            ...
            // Deadline bar + instant-reveal flash — cheap overlay, repaints off
            // _frame, reads live sim state. Never repaints the heavy slide.
            Positioned.fill(
              child: CustomPaint(
                painter: _SlideOverlayPainter(this, _frame),
              ),
            ),
            // Scope label.
            Positioned(
  ...
  // -- Fact ticker (persistent, fixed slot, non-blocking) ----------------------

  Widget _buildTicker() {
    // A single leaf rebuilt per frame to cross-fade the fact against the
    // neutral scaffold hint — never rebuilds the play tree.
    return ValueListenableBuilder<int>(
      valueListenable: _frame,
      builder: (context, _, __) {
        final strength = (_factLife / 0.6).clamp(0.0, 1.0);
        final hasFact = _factTissue != null && strength > 0.01;
        ...
        return Container(
          ...
          child: Stack(
            children: [
              // Neutral scaffold hint (what newcomers should look for).
              Opacity(
                opacity: (1.0 - strength).clamp(0.0, 1.0),
                child: _tickerShell(
                  border: _kCardBorder.withValues(alpha: 0.6),
                  bg: _kCardBg.withValues(alpha: 0.5),
                  child: Center(
                    child: Text(
                      'READ THE SHAPE  ·  sheet · scatter · fibre · star',
                      maxLines: 1,
                      overflow: TextOverflow.ellipsis,
                      style: const TextStyle(
                        fontFamily: _kFont,
              ...
              // The fact for the sample just answered (fades on its own life).
              if (hasFact)
                Opacity(
                  opacity: strength,
  ...
  Widget _buildFactCard() {
    final meta = _kMeta[_factTissue]!;
    final subName = meta.subtypeNames[_factSubtype];
    final accent = _factCorrect ? _kGoodGreen : _kBadRed;
    final header = _factCorrect
        ? (_factMult > 1 ? '+$_factEarned  ×$_factMult' : '+$_factEarned')
        : (_factTimedOut ? 'TIME!' : 'MISS');
            ...
            children: [
              ...
              Text(
                ...
                style: TextStyle(
                  ...
                  fontWeight: FontWeight.w700,
                  letterSpacing: 0.5,
                  color: meta.accent.withValues(alpha: 0.95),
                ),
              ),
            ],
          ),
...
// ============================================================================
// Ambient background + particles (cheap, repaints off _frame)
// ============================================================================

class _AmbientPainter extends CustomPainter {
  final _TissueTypeV2GameState state;
  _AmbientPainter(this.state, Listenable repaint) : super(repaint: repaint);
...
// ============================================================================
// Slide overlay — per-sample deadline bar + instant-reveal flash.
// Cheap; repaints off _frame; reads live sim state. NEVER touches the heavy
// _SamplePainter (which stays cached behind its RepaintBoundary).
// ============================================================================

class _SlideOverlayPainter extends CustomPainter {
  final _TissueTypeV2GameState state;
  _SlideOverlayPainter(this.state, Listenable repaint)
      : super(repaint: repaint);

  @override
  void paint(Canvas canvas, Size size) {
    if (state._answer == _Answer.waiting) {
      // Deadline bar drains across the top while the player decides.
      final frac =
          (1.0 - (state._sampleTime / state._deadline)).clamp(0.0, 1.0);
      final barColor = frac < 0.3 ? _kBadRed : _kMembrane;
      canvas.drawRect(
        Rect.fromLTWH(0, 0, size.width * frac, 4),
        Paint()..color = barColor,
      );
    } else {
      // Instant-confirmation flash: a quick correct/wrong wash that fades over
      // the short reveal window — the buzzer rhythm is never stopped.
      final a = (state._revealTimer / _kRevealDuration).clamp(0.0, 1.0);
      final c = state._answer == _Answer.correct ? _kGoodGreen : _kBadRed;
      canvas.drawRect(
        Offset.zero & size,
        Paint()..color = c.withValues(alpha: 0.16 * a),
      );
    }
  }

  @override
  bool shouldRepaint(_SlideOverlayPainter old) => false;
}

// ============================================================================
// Sample painter — the procedurally-drawn histology slide. PRESERVED from v1
// verbatim: this is the brand asset of the game.
